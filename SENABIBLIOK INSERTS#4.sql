...
starterdbSELECT * FROM emprestimoslivros

/* Criar uma view de quantos livros cada autor escreveu
Tudo isto em ordem alfabética por nome do autor*/

CREATE VIEW vw_livrosQtd_NomeAutor AS
SELECT autores.nome, livros.nome, livrosautores.idLivro, livrosautores.idAutor
FROM livros
INNER JOIN
livrosautores
ON livros.idLivro = livrosautores.idLivro 
INNER JOIN
autores
ON livrosautores.idAutor = autores.idAutor
